...
//    @PostMapping("/update")
    ...
    public Result delete(String id) {
        ...
        try {
            ...
            result.setMsg("删除帮扶镇信息成功!");
        } catch (Exception e) {
            e.printStackTrace();
            result.setState(false).setMsg("删除帮扶镇信息失败!");
        }
        return result;
    }
...
//    @PostMapping("/save")
    ...
    public Map<String, Object> findAllPlace(Integer page, Integer rows, String cityId) {
        HashMap<String, Object> map = new HashMap<>();
        page = page == null ? 1 : page;
        rows = rows == null ? 4 : rows;
        // 景点集合
        List<Place> places = placeService.findByCityIdPage(page, rows, cityId);

        // 处理分页
        Integer counts = placeService.findByCityIdCounts(cityId); // 总页数
